// SE SUA IDE NÃO COMPILAR NO BOTÃO DE COMPILAR RODE 'gcc server.c -o server.exe -lws2_32' NO TERMINAL
// RODE PRIMEIRO O SERVER.EXE DEPOIS O CLIENT.EXE
...
int main() {
    ...
    printf(GREEN "Cliente conectado!\n" RESET);
    Sleep(1000);
        ...
        if (myTurn) {
            ...
            if (myGrid[r][c] == 'S') {
                ...
                if (checkSunkAt(r, c)) {
                    shipsRemaining--;
                    if (shipsRemaining <= 0) {
                        send(client, "SUNK ALL", 8, 0);
                        showDefeatScreen();
                        break;
                    } else
                        send(client, "SUNK", 4, 0);
                } else
                    send(client, "HIT", 3, 0);
            } else {
                myGrid[r][c] = 'M';
                send(client, "MISS", 4, 0);
            }
